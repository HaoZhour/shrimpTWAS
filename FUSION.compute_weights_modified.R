...
weights.lasso = function( input , hsq , snp , out=NA ) {
	...
	arg = paste( opt$PATH_plink , " --allow-no-sex --allow-extra-chr --bfile " , input , " --lasso " , hsq , " --out " , out , sep='' )
	system( arg , ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
if ( !is.na(opt$pheno) ) {
	...
	pheno = read.table(pheno.file,as.is=T, header = TRUE)
	# Match up data
...
arg = paste( opt$PATH_plink ," --allow-no-sex --bfile ",opt$bfile," --pheno ",pheno.file," --allow-extra-chr  --make-bed --out ",geno.file,sep='')
system(arg,ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
# 1. generate GRM
arg = paste( opt$PATH_plink," --allow-no-sex --bfile ",opt$tmp," --make-grm-bin --allow-extra-chr --out ",opt$tmp,sep='')
system(arg,ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
...
for ( i in 1:opt$crossval ) {
	...
	arg = paste( opt$PATH_plink ," --allow-no-sex --allow-extra-chr  --bfile ",opt$tmp," --keep ",cv.file,".keep --out ",cv.file," --make-bed",sep='')
	system(arg, ignore.stdout=SYS_PRINT,ignore.stderr=SYS_PRINT)
